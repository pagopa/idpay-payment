...
The new classification belongs in the outbox `event_type` column and the
corresponding Kafka event type or header.
...
If the current CDC integration exposes outbox `event_type` through a Kafka
header named `operationType`, that header can be retained for compatibility,
but it must remain distinct from the payload's `operationType` field. The
contract must document this distinction explicitly.
...
### 4. Use one versioned event envelope
...
- has the same revision as the envelope;
...
If replacing the existing raw `RewardTransactionDTO` payload with an envelope
is not wire-compatible, the same metadata may initially be represented through
additive payload fields or Kafka headers. Regardless of representation, one
outbox row and one Kafka message remain the invariant.
...
- existing consumers must understand or intentionally ignore the new event
  type;
- introducing a versioned envelope may require a compatibility rollout;
...
2. revise
   ...
   separately; and
3. update `specs/asyncapi.yml` with the versioned event envelope, the new event
   type, transaction key, and compatibility rules.
