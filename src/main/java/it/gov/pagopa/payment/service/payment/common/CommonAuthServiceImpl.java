...
public abstract class CommonAuthServiceImpl {
        ...
        if (trx.getStatus().equals(SyncTrxStatus.IDENTIFIED)) {
            ...
            if(SyncTrxStatus.REWARDED.equals(authPaymentDTO.getStatus())) {
                ...
                if (authPaymentDTO.getRejectionReasons().contains(RewardConstants.INITIATIVE_REJECTION_REASON_BUDGET_EXHAUSTED)) {
                    throw new ForbiddenException(PaymentConstants.ExceptionCode.BUDGET_EXHAUSTED,
                        "Budget exhausted for user [%s] and initiative [%s]".formatted(userId, trx.getInitiativeId()));
                }
                throw new ForbiddenException(PaymentConstants.ExceptionCode.REJECTED,
            ...
            throw new ForbiddenException(PaymentConstants.ExceptionCode.TRX_ALREADY_AUTHORIZED,
                "Transaction with trxCode [%s] is already authorized".formatted(trxCode));
        } else {
            throw new BadRequestException(PaymentConstants.ExceptionCode.TRX_STATUS_NOT_VALID,
    ...
    private void checkWalletStatus(String initiativeId, String userId){
        ...
        if (PaymentConstants.WALLET_STATUS_SUSPENDED.equals(walletStatus)){
            throw new ForbiddenException(PaymentConstants.ExceptionCode.USER_SUSPENDED_ERROR,
    ...
    private void checkAuth(String trxCode, String userId, TransactionInProgress trx){
        if (trx == null) {
            throw new NotFoundException(PaymentConstants.ExceptionCode.TRX_NOT_FOUND_OR_EXPIRED,
                "Cannot find transaction with trxCode [%s]".formatted(trxCode));
        }

        if (trx.getUserId()!=null && !userId.equals(trx.getUserId())) {
            throw new ForbiddenException(PaymentConstants.ExceptionCode.TRX_ANOTHER_USER,
