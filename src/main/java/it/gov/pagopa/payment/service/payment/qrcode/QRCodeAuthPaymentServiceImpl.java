...
public class QRCodeAuthPaymentServiceImpl implements QRCodeAuthPaymentService {
  ...
  public AuthPaymentDTO authPayment(String userId, String trxCode) {
    Transaction transaction = qrCodeAuthorizationExpiredService.findByTrxCodeAndAuthorizationNotExpired(trxCode.toLowerCase());
    checkUser(trxCode,userId,transaction);
    return commonAuthService.authPayment(transaction,userId,trxCode);
  }

  private void checkUser(String trxCode,String userId, Transaction transaction){
