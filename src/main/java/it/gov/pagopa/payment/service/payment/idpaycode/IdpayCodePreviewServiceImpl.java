...
public class IdpayCodePreviewServiceImpl implements IdpayCodePreviewService{
    ...
    public AuthPaymentDTO previewPayment(String trxId, String merchantId) {
        ...
        if(transaction.getUserId() == null){
            return authPaymentMapper.transactionMapper(transaction);
        }

        SecondFactorDTO secondFactorDetails = paymentInstrumentConnector.getSecondFactor(transaction.getUserId());

        commonPreAuthService.checkPreAuth(transaction.getUserId(), transaction);
        AuthPaymentDTO authPaymentDTO = commonPreAuthService.previewPayment(transaction, RewardConstants.TRX_CHANNEL_IDPAYCODE, SyncTrxStatus.IDENTIFIED);

        auditUtilities.logPreviewTransaction(transaction.getInitiativeId(), transaction.getId(), transaction.getTrxCode(), transaction.getUserId(), RewardConstants.TRX_CHANNEL_IDPAYCODE);
