...
public class QRCodeUnrelateServiceImpl implements QRCodeUnrelateService{
    ...
    public void unrelateTransaction(String trxCode, String userId) {
        try {
            ...
            if (trx == null) {
                throw new TransactionNotFoundOrExpiredException(ExceptionCode.TRX_NOT_FOUND_OR_EXPIRED, "[UNRELATE_TRANSACTION] Cannot find transaction having code: %s".formatted(trxCode));
            }

            if (SyncTrxStatus.IDENTIFIED.equals(trx.getStatus())) {
                if(!trx.getUserId().equals(userId)){
                    throw new UserNotAllowedException(ExceptionCode.PAYMENT_USER_NOT_VALID, "[UNRELATE_TRANSACTION] Requesting userId (%s) not allowed to operate on transaction having id %s".formatted(userId, trx.getId()));
                }
                ...
                throw new OperationNotAllowedException(ExceptionCode.TRX_STATUS_NOT_VALID, "[UNRELATE_TRANSACTION] Cannot unrelate transaction not in status IDENTIFIED: id %s".formatted(trx.getId()));
            }
