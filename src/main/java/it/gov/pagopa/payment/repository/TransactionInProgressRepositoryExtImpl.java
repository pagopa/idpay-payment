...
public class TransactionInProgressRepositoryExtImpl implements TransactionInProgressRepositoryExt {
  ...
  public UpdateResult createIfExists(TransactionInProgress trx, String trxCode) {
    ...
    return mongoTemplate.upsert(
        ...
        new Update()
            ...
            .setOnInsert(Fields.effectiveAmountCents,
                trx.getAmountCents() != null ? trx.getAmountCents() : null)
        ...
        .orOperator(
            ...
            Criteria.expr(
                ComparisonOperators.Lt.valueOf(Fields.trxChargeDate)
                    .lessThan(ArithmeticOperators.Subtract.valueOf(
                            MongoConstants.AGGREGATION_EXPRESSION_VARIABLE_NOW)
                        .subtract(1000 * trxThrottlingSeconds))));
  }

  @Override
  public void updateTrxRejected(String id, String userId, List<String> rejectionReasons,
      Map<String, List<String>> initiativeRejectionReasons, String channel) {
  ...
  @Override
  public void updateTrxWithStatusForPreview(TransactionInProgress trx, AuthPaymentDTO preview,
      Map<String, List<String>> initiativeRejectionReasons, String channel, SyncTrxStatus status) {
    mongoTemplate.updateFirst(
        Query.query(Criteria.where(Fields.id).is(trx.getId())),
        new Update()
  ...
  @Override
  public UpdateResult updateTrxAuthorized(TransactionInProgress trx, AuthPaymentDTO authPaymentDTO,
    ...
    return mongoTemplate.updateFirst(
        Query.query(Criteria.where(Fields.id).is(trx.getId()).and(Fields.status)
  ...
  public void updateTrxRejected(TransactionInProgress trx, List<String> rejectionReasons,
      Map<String, List<String>> initiativeRejectionReason) {
    Update update = new Update()
        .set(Fields.status, SyncTrxStatus.REJECTED)
        .set(Fields.rewardCents, 0L)
        .set(Fields.rewards, Collections.emptyMap())
  ...
  public List<TransactionInProgress> findByFilter(Criteria criteria, Pageable pageable) {
    return mongoTemplate.find(Query.query(criteria).with(CommonUtilities.getPageable(pageable)),
        TransactionInProgress.class);
  }
  ...
  public Page<TransactionInProgress> findPageByFilter(String merchantId, String pointOfSaleId,
    ...
    if (aggregation != null) {
      transactions = mongoTemplate.aggregate(aggregation, TransactionInProgress.class,
              TransactionInProgress.class)
          .getMappedResults();
    } else {
  ...
  @Override
  public TransactionInProgress findCancelExpiredTransaction(String initiativeId,
      long cancelExpirationMinutes) {
    return findExpiredTransaction(initiativeId, cancelExpirationMinutes,
        List.of(SyncTrxStatus.AUTHORIZED));
  }

  @Override
  public TransactionInProgress findAuthorizationExpiredTransaction(String initiativeId,
      long authorizationExpirationMinutes) {
    return findExpiredTransaction(initiativeId, authorizationExpirationMinutes,
        List.of(SyncTrxStatus.IDENTIFIED, SyncTrxStatus.CREATED, SyncTrxStatus.REJECTED));
  }
  ...
  public UpdateResult updateTrxPostTimeout(String trxId) {
    Query query = new Query(Criteria.where(Fields.id).is(trxId).and(Fields.status)
        .is(SyncTrxStatus.AUTHORIZATION_REQUESTED));
    Update update = new Update()
        .set(Fields.status, SyncTrxStatus.REJECTED)
