...
class IdpayCodePreAuthServiceTest {
    ...
    void previewPayment() {
        ...
        doNothing().when(transactionInProgressRepositoryMock)
                .updateTrxWithStatus(
                        ...
                        Mockito.any(), eq(authPaymentDTO.getAmountCents()), eq(trx.getMerchantId()));
        ...
        verify(transactionInProgressRepositoryMock, times(1)).updateTrxWithStatus(anyString(), anyString(), anyLong(), eq(Collections.emptyList()), anyMap(), anyMap(),anyString(), any(),anyLong(),any(), anyLong(), any());
    ...
    void previewPayment_RejectedStatusRE() {
        ...
        verify(transactionInProgressRepositoryMock, times(1)).findById(anyString());
        verify(rewardCalculatorConnectorMock, times(1)).previewTransaction(any());
        verify(transactionInProgressRepositoryMock, times(0)).updateTrxWithStatus(anyString(), anyString(), anyLong(), eq(null), anyMap(), anyMap(),anyString(),any(),anyLong(),any(), anyLong(), any());
