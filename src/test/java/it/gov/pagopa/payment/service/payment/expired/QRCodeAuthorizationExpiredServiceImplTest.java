...
class QRCodeAuthorizationExpiredServiceImplTest {
    ...
    void handleExpiredTransaction() {
        ...
        Mockito.when(rewardCalculatorConnectorMock.cancelTransaction(trxIdentifiedException404)).thenThrow(new TransactionNotFoundOrExpiredException("NOT_FOUND", "NOT_FOUND"));
        Mockito.when(rewardCalculatorConnectorMock.cancelTransaction(trxIdentifiedException500)).thenThrow(new RewardCalculatorInvocationException("INTERNAL_SERVER_ERROR", "INTERNAL_SERVER_ERROR"));
